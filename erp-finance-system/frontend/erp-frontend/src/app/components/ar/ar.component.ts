    ...
    <div class="module-section">
      <div class="section-header">
        ...
        <div class="section-actions">
          ...
          <button class="btn btn-success" (click)="sendReminders()">
            <i class="fas fa-envelope"></i> Send Reminders
          </button>
          <button class="btn btn-info" (click)="exportARData()">
      ...
      <div class="cards-container">
        <div class="card">
          ...
          <div class="card-value">{{ formatCurrency(getTotalReceivables()) }}</div>
          <div class="card-trend positive">
            <i class="fas fa-arrow-up"></i> Outstanding
          </div>
        </div>
        <div class="card">
          <div class="card-header">
        ...
        <div class="card">
          ...
          <div class="card-value">{{ formatCurrency(getCollectedThisMonth()) }}</div>
          <div class="card-trend positive">
      ...
      <div class="search-filter">
        <div class="search-box">
          ...
          <input type="text" class="form-control" placeholder="Search invoices..." [(ngModel)]="searchTerm" (input)="searchAR()">
        </div>
        <select class="form-control filter-dropdown" [(ngModel)]="filterStatus" (change)="filterAR()">
        ...
        <select class="form-control filter-dropdown" [(ngModel)]="filterCustomer" (change)="filterAR()">
          <option value="">All Customers</option>
          <option *ngFor="let customer of getUniqueCustomerList()" [value]="customer">{{ customer }}</option>
        </select>
      </div>
          ...
          <tbody>
            <tr *ngFor="let invoice of filteredInvoices">
              ...
              <td>
                ...
                <button class="btn btn-sm btn-success" *ngIf="invoice.status !== 'Paid'" (click)="recordPayment(invoice.id)">
                  <i class="fas fa-dollar-sign"></i>
                </button>
              </td>
            </tr>
          ...
          <form #invoiceForm="ngForm">
            <div class="form-row">
              <div class="form-group">
                ...
                <select class="form-control" [(ngModel)]="invoiceFormData.customerId" name="customerId" required>
...
export class ArComponent implements OnInit {
  ...
  customers = [
    { id: 1, name: 'ABC Corporation', email: 'billing@abc.com', phone: '555-0101' },
    { id: 2, name: 'XYZ Industries', email: 'accounts@xyz.com', phone: '555-0102' },
    { id: 3, name: 'Global Services Ltd.', email: 'finance@global.com', phone: '555-0103' },
    { id: 4, name: 'Tech Solutions Inc.', email: 'billing@techsol.com', phone: '555-0104' }
  ];
  ...
  loadInvoices() {
    const saved = this.hasLocalStorage() ? localStorage.getItem('erpARInvoices') : null;
    if (saved) {
  ...
  saveInvoices() {
    if (this.hasLocalStorage()) {
      localStorage.setItem('erpARInvoices', JSON.stringify(this.invoices));
    }
  }
  ...
  filterInvoices() {
    this.filteredInvoices = this.invoices.filter(invoice => {
      ...
      const matchesCustomer = !this.filterCustomer || invoice.customerName === this.filterCustomer;
  ...
  getUniqueCustomers(): number {
    return new Set(this.invoices.map(inv => inv.customerId)).size;
  }

  getUniqueCustomerList(): string[] {
    return [...new Set(this.invoices.map(inv => inv.customerName))];
  }
  ...
  saveInvoice() {
    const customer = this.customers.find(c => c.id.toString() === this.invoiceFormData.customerId);
    if (!customer) return;
  ...
  sendReminders() {
    ...
    alert(`Sent payment reminders for ${overdueInvoices.length} overdue invoices`);
  }
